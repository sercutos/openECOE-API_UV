...
def export_csv(identidad, ecoe="", organization=""):
    ...
    try:
        ...
        # 1. Leemos la tabla student primero
        df_tmp_student = pd.read_sql_table("student", conexion)

        # 2. Si ya existe la columna destino, la borramos para evitar el conflicto
        if 'student_planner_order' in df_tmp_student.columns:
            df_tmp_student = df_tmp_student.drop(columns=['student_planner_order'])

        # 3. Ahora hacemos el merge con el rename seguro
        ...
        df_student = pd.merge(
            left=df_planner, 
            right=df_tmp_student.rename(columns = {
                'id':'id_student',
                'name':'student_name',
                'surnames':'student_surnames',
                'dni':'student_dni',
                'planner_order':'student_planner_order'
            }),
            on=['id_planner','id_ecoe']
        )
