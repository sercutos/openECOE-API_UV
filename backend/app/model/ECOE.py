...
class ECOE(db.Model):
    ...
    def configuration(self):
        ...
        try:
            #time_start = self.shifts[0].time_start
            if self.shifts and self.shifts[0].time_start:
                time_start_obj = self.shifts[0].time_start + timedelta(hours=1)
                time_start = time_start_obj.strftime('%Y-%m-%d %H:%M:%S')
        
        except Exception as e:
            print(f"Error calculando time_start: {e}")
            time_start = ""
